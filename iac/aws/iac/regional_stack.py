...
class RegionalStack(aws_cdk.Stack):
    def __init__(self, scope,  application_ci: str,
                 source_bucket: BucketAttributes, target_buckets: List[BucketAttributes], 
                 source_scratch_bucket: BucketAttributes, target_scratch_buckets: List[BucketAttributes],
                 deploy_replication: bool,
                 app_environment: str, credential_role_level: str, tag: str,
                 aws_environment: aws_cdk.Environment, id: builtins.str, **kwargs):
        # application_ci=application_ci
        super().__init__(scope, id)
        prefix = ""
        if (app_environment != "dev"):
            prefix = "ADFS-"
        dev_role = aws_iam.Role.from_role_name(self, "ebrDevRole",
                                               role_name=f"""{prefix}-{app_environment.upper()}-OperationsTechnology_EBR-{credential_role_level.upper()}""")

        
        datalake_stack = IotDataLakeStack(self, "datalake",
                                                application_ci=application_ci,
        ...
        scratch_stack = IotScratchStack(self, "scratch",
                                                application_ci=application_ci,
